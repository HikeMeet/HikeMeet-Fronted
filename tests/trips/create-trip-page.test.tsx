...
describe("CreateTripPage", () => {
  ...
  it("creates trip successfully with all required data", async () => {
    const { getAllByText, getByText, getByPlaceholderText, getByTestId } =
      render(<CreateTripPage navigation={mockNavigation} {...({} as any)} />);

    // Fill in trip name
    const nameInput = getByPlaceholderText("Name");
    fireEvent.changeText(nameInput, "My Test Trip");

    // Fill in description
    const descriptionInput = getByPlaceholderText("Description (optional)");
    fireEvent.changeText(descriptionInput, "Test trip description");
    ...
    fireEvent.press(getByText("Mountains"));
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining("/api/trips/create"),
        expect.objectContaining({
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: expect.stringContaining("My Test Trip"),
        })
      );
    });

    // Check the request body contains expected data
