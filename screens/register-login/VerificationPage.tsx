...
export default function VerificationPage({ route, navigation }: { route: any; navigation: any }) {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.EXPO_BASE_IP}/api/user/verify-code`, {
        method: "POST",
        headers: {
  ...
  const handleResendCode = async () => {
    try {
      ...
      const response = await fetch(`${process.env.EXPO_BASE_IP}/api/user/send-verification-code`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
